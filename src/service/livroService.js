...
export function criarLivro(body) {
  ...
  if (!nome || String(nome).trim() === '') erros.push('Nome do livro é obrigatório.');

  const autoresArr = normalizeAutores(autores);
  if (autoresArr.length < 1) erros.push('Informe ao menos um autor.');

  if (ano_publicacao === undefined || ano_publicacao === null || ano_publicacao === '') {
  ...
  if (qtde_disponivel === undefined || qtde_disponivel === null || qtde_disponivel === '') {
  ...
  const ed = edicao === undefined || edicao === null || edicao === '' ? 1 : Number(edicao);
  ...
  if (signatures.has(sig)) {
    const err = new Error('CONFLICT');
    err.status = 409;
    err.mensagem = 'Já existe cadastro para a combinação título, autor, ano e edição.';
    throw err;
  }
...
function normalizarLivroPayload(body, { isUpdate = false } = {}) {
  ...
  if (!isUpdate || id_livro !== undefined) {
    if (!id_livro || String(id_livro).trim() === '') erros.push('Campo id_livro (ISBN) é obrigatório.');
  }
  ...
  if (!isUpdate || qtde_disponivel !== undefined) {
    if (qtde_disponivel === undefined || qtde_disponivel === null || qtde_disponivel === '') {
      ...
      const q = Number(qtde_disponivel);
      if (!Number.isInteger(q) || q <= 0) erros.push('Quantidade de exemplares deve ser um inteiro positivo.');
    }
  }
  ...
  if (!isUpdate || categoria !== undefined) {
    ...
    if (categoria !== undefined && categoria !== null && categoria !== '') {
      if (Array.isArray(categoria)) {
        ...
        categorias = String(categoria).split(',').map((c) => c.trim()).filter(Boolean);
      }
      if (categorias.length > 3) erros.push('No máximo 3 categorias por livro.');
    }
  }

  if (!isUpdate || edicao !== undefined) {
    const ed = edicao === undefined || edicao === null || edicao === '' ? 1 : Number(edicao);
    if (!Number.isInteger(ed) || ed < 1) erros.push('Edição deve ser um inteiro positivo.');
  }
...
export function atualizarLivro(isbnParam, body) {
  ...
  const novoIsbn = payload.id_livro !== undefined ? String(payload.id_livro).trim() : livro.id_livro;
  ...
  const nome = payload.nome !== undefined ? String(payload.nome).trim() : livro.nome;
  const autoresArr = payload.autoresArr !== null ? payload.autoresArr : normalizeAutores(livro.autores);
  const ano = payload.ano_publicacao !== undefined ? Number(payload.ano_publicacao) : livro.ano_publicacao;
  const ed = payload.edicao !== undefined ? Number(payload.edicao) : livro.edicao;

  const assinaturaAtual = livroSignature(livro.nome, normalizeAutores(livro.autores), livro.ano_publicacao, livro.edicao);
  ...
  if (payload.ano_publicacao !== undefined) livro.ano_publicacao = Number(payload.ano_publicacao);
  if (payload.edicao !== undefined) livro.edicao = Number(payload.edicao);
  if (payload.paginas !== undefined) livro.paginas = payload.paginas === '' || payload.paginas === null ? null : Number(payload.paginas);
  if (payload.qtde_disponivel !== undefined) livro.qtde_disponivel = Number(payload.qtde_disponivel);
  if (payload.categorias !== null) livro.categoria = payload.categorias;
  if (payload.editora !== undefined) livro.editora = payload.editora === null ? null : String(payload.editora);
  if (payload.idioma !== undefined) livro.idioma = payload.idioma === null ? null : String(payload.idioma);
...
export function removerLivro(isbn) {
  ...
  const assinatura = livroSignature(livro.nome, normalizeAutores(livro.autores), livro.ano_publicacao, livro.edicao);
